...
$scope.allApplications = result;  
  ...
  $scope.getUpdatedApplication = function() {
   ...
   $http.get('/user/applications/'+ $rootScope.currentUser._id)
...
//Payslip.query({pie:$scope.personalInfo.uuid}, function(result){
  $http.get('/user/payslips/'+ $scope.personalInfo.uuid).then(function(result){
...
$scope.personalInfo.status = 1;
  ...
  $scope.validateApplication = function () {
    ...
    if($scope.personalInfo.academics.length < 2) {
      return $scope.applicationErrorMsg ='Your academic history cannot be empty';
    } 

    if ($scope.personalInfo.refs.length < 3) {
...
$scope.personalInfo.DOB = $filter('date')($scope.personalInfo.DOB, "dd/MM/yyyy");
 console.log($scope.personalInfo.DOB);
  $scope.personalInfo.status = 1;
...
$http.put('/user/appusers/'+ $rootScope.currentUser._id, {status:1}); 
...
$http.put('/user/appusers/'+ $rootScope.currentUser._id, {picUrl:file.name});
                  }
